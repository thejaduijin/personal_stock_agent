...
# --------------------------------------------------------------------------
# Single Stock Search Endpoint
# --------------------------------------------------------------------------

@app.route("/search_stock", methods=["POST"])
def search_stock():
    """
    Analyze a single stock symbol through the full agent pipeline.
    Fetches live data, runs all agents (Technician, Fundamentalist, Newsdesk),
    conducts Bull vs Bear debate, and returns the Judge's verdict.
    """
    payload = request.get_json(silent=True) or {}
    ...
    # Auto-append .NS if not present (NSE convention)
    if "." not in symbol:
        symbol = symbol + ".NS"

    log_line(f"Search: analyzing {symbol}...")

    try:
        # Step 1: Fetch evidence bundle
        ...
        # Step 2: Run the full evaluation (LLM first, fallback to deterministic)
        try:
        ...
        log_line(f"Search: {symbol} -> {verdict.get('verdict', 'WATCH')} ({verdict.get('confidence', 0)}/10) via {engine_used}")
        return jsonify(response)

    except Exception as e:
        log_line(f"Search: ERROR analyzing {symbol}: {e}")
